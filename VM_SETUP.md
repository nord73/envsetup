...
- [Overview](#overview)
- [VM Platform Preparation](#vm-platform-preparation)
...
## VM Platform Preparation

### Creating the Base VM

#### VMware Workstation/Fusion
1. Create new VM
2. Select Ubuntu Server ISO (22.04 LTS or 24.04 LTS recommended)
3. Allocate resources:
   - **Headless server**: 2GB RAM, 1-2 CPU cores
   - **Desktop VM**: 4GB+ RAM, 2-4 CPU cores
   - **Developer VM**: 8GB+ RAM, 4+ CPU cores
4. Storage: 25GB minimum, 50GB+ recommended for development
5. Enable 3D acceleration if using desktop environment

#### VirtualBox
1. Create new VM (Type: Linux, Version: Ubuntu 64-bit)
2. Allocate memory and CPUs (see recommendations above)
3. Create virtual hard disk (VDI, dynamically allocated)
4. Settings → Display → Enable 3D acceleration
5. Settings → Network → Adapter 1: NAT or Bridged

#### Hyper-V
1. Quick Create → Ubuntu (or manual VM creation)
2. Generation 2 VM recommended
3. Dynamic memory: Enable, minimum 2GB
4. Network: Default Switch or External Network
5. Integration Services: Enable

#### KVM/QEMU (virt-manager)
1. Create new VM from ISO
2. Select appropriate OS type: Ubuntu
3. Allocate resources
4. Network: Default NAT or Bridge
5. Enable VirtIO for better performance

### Initial OS Installation

**Installation Options:**
1. **Ubuntu Server**: Minimal, fastest, recommended for headless
2. **Ubuntu Desktop**: Full desktop environment
3. **Debian**: Stable, minimal base

**During Installation:**
- Create user account (e.g., `dev`, `admin`, or your name)
- Enable OpenSSH server (recommended)
- Do NOT install additional packages yet (we'll use envsetup)
- Allow it to install security updates
...
# Install guest additions (if not auto-installed)
# This varies by hypervisor - see VM Optimization section
